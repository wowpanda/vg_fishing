...
function GenerateShopMenu()
	...
	local fishingrod = NativeUI.CreateItem("Angelrute", "1,85m für Boote")
    shopMenu:AddItem(fishingrod)
	
	local fishingknife = NativeUI.CreateItem("Anglermesser", "Zum Verarbeiten von Fisch")
    shopMenu:AddItem(fishingknife)
	
	local bait1 = NativeUI.CreateItem("Fischköder", "für kleine bis mittlere Fische")
    shopMenu:AddItem(bait1)
	
	local bait2 = NativeUI.CreateItem("Schildkrötenfutter", "Kann auch als Köder benutzt werden")
...
Citizen.CreateThread(function() 
    while true do
		...
		if IsControlJustPressed(1, 81) and not IsPedInAnyVehicle(playerPed) and boat ~= nil then
